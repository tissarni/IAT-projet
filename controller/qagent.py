...
class QAgent():
    ...
    def __init__(self, env, eps_profile: EpsilonProfile, gamma: float, alpha: float):
        """A LIRE
    ...
    def learn(self, env, n_episodes, max_steps):
        ...
        for episode in range(n_episodes):
            ...
            if not state in self.Q: 
                self.Q[state] = [0. for _ in range(self.na)]
            ...
            for step in range(max_steps):
                ...
                if not next_state in self.Q: 
                    self.Q[next_state] = [0. for _ in range(self.na)]
    ...
    def save_log(self, env, episode):
        ...
        # V = np.zeros((int(self.env.ny), int(self.env.nx)))
        ...
        # self.values = self.values.append({'episode': episode, 'value': np.reshape(V,(1, self.env.ny*self.env.nx))[0]},ignore_index=True)