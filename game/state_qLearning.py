...
class State():
	...
	def update_state(self, invaders_x, invaders_y, player_x):
		...
		for i in range(self.nbInvaders):
			...
			zone_x = self.getZone("x", player_x, (invaders_x, invaders_y))
			zone_y = self.getZone("y", player_x, (invaders_x, invaders_y))
	...
	def getZone(self, s, player_x, invaders_i):
		...
		if (s=="y"):
			if (abs(self.player_y-invaders_i[1])<200):
				return "veryCloseZone"
			if (abs(self.player_y-invaders_i[1])<300 and abs(self.player_y-invaders_i[1])<200):
				return "midCloseZone"
			return "farCloseZone"